...
class RoomReader(XmlReader):
    ...
    def parse(self):
        ...
        for room in self.xml.getroot():
            ...
            aRoom.libelle = room[1].text
            aRoom.imageBackground = room[2].text
            aRoom.imageChar = room[3].text
            aRoom.xMin = float(room[4].attrib['min'])
            aRoom.xMax = float(room[4].attrib['max'])
            aRoom.yMin = float(room[5].attrib['min'])
            aRoom.yMax = float(room[5].attrib['max'])
            aRoom.score = int(10)
